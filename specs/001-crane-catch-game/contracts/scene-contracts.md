...
3. キャッチ: ボタンタップ → **リング開く（open）** → リング下降 → **リング閉じる（close）** → X-Z 2D距離で掴み判定 → 上昇
4. 結果判定: CatchSystem が成功/失敗を決定
5. 遷移: result シーンへ

**BGM**: `enter()` 内で `audioManager.playBGM('game')` を呼び出し。`exit()` で `stopBGM()`（フェードアウト）。
...
**表示要素**:
- カテゴリタブ（横スクロール）
- アイテムグリッド（**4列固定: `repeat(4, 1fr)`、2行×4列マトリックス**）:
  - 取得済み: カラー表示、タップで詳細
  - 未取得: シルエット + ？マーク
